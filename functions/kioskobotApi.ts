...
Deno.serve(async (req) => {
  try {
    const base44 = createClient({ appId: QUICKORDER_APP_ID });
    ...
    if (action === 'get_categories') {
      const records = await db.entities.Category.filter({ active: true });
